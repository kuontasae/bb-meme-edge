...
bb Meme Edge は、Nansen Smart Money flowを使って、Solana meme coinの早期兆候を検出するDiscord Botです。Fresh Scanで市場候補を裏側に保存し、Alertで今見るべき候補だけを自動通知します。
...
ユーザーはAlert Cardの Conviction / エアIN / Watch でPick参加でき、`/my` や `/leaderboard` で成績確認できます。特徴は、Nansen CLIで得たwallet情報を保存し、後追い成績と紐づけてSmart Wallet Qualityを育てる点です。さらにAuto-Tuningで過去に強かった特徴を次回AlertのFinal Scoreへ安全に反映し、検出 → 後追い → 学習 → 改善のPDCAが回る設計にしています。
...
この設計はNansen creditsを増やさず、既存CLI結果をbb Meme Edge独自のSmart Wallet Qualityへ変換するものです。将来的には、bb Meme Edge独自のSmart Walletランキングへ発展できます。
...
Fresh Scan、Deep Check、Nansen credit logging、Alert手動実行、開発用リセットなどはユーザー向けコマンドではありません。裏側の自動処理・運用機能として残し、提出版では初見ユーザーが結果確認・参加・答え合わせに集中できる構成にしています。
...
2. Alert Cardの Conviction / エアIN / Watch を押す  
   ユーザーは実取引ではなくPaper Pickとして参加できます。
